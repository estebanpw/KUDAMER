...
void init_args(int argc, char ** av, FILE ** query, FILE ** ref, FILE ** out, unsigned * write, uint64_t * n_threads);
...
{
    uint64_t i, n_threads = 4;
    unsigned write = 0;
    FILE * query = NULL, * ref = NULL, * out = NULL;
    init_args(argc, argv, &query, &ref, &out, &write, &n_threads);
    ...
    if(threads == NULL) { fprintf(stderr, "Could not create threads"); exit(-1); }
    ...
    while(!feof(query)){
        ...
        Arguments arguments[n_threads];

        for(i=0;i<n_threads;i++){
            arguments[i].seq = query_mem_host;
            arguments[i].table = table_mem;
            arguments[i].len = (items_read / n_threads);
            arguments[i].offset = i*(items_read / n_threads);

            if( 0 != (ret = pthread_create(&threads[i], NULL, parallel_launcher, (void *) (&arguments[i])) )){
                fprintf(stderr, "Thread %"PRIu64" returned %d:", i, ret); exit(-1);
            }
        }
...
void init_args(int argc, char ** av, FILE ** query, FILE ** ref, FILE ** out, unsigned * write, uint64_t * n_threads){
    ...
    while(pNum < argc){
        if(strcmp(av[pNum], "--help") == 0){
            ...
            fprintf(stdout, "           -threads    Number of threads to split input across\n");
...
void perfect_hash_to_word(char * word, uint64_t hash, uint64_t k){
    ...
    uint64_t v;
    while(jIdx >= 0){
        v = (uint64_t) floor(hash / (pow( (uint64_t) 4, jIdx)));
...
void * parallel_launcher(void * arguments){
    ...
    char * sequence = args->seq;
    uint64_t * table_mem = args->table;
    uint64_t len = args->len;
    uint64_t offset = args->offset;

    compute_kmers(&sequence[offset], &table_mem[offset], len);
